...
/// @file
/// Implementation for class property_disc_iterator_4_3
...
// ===========================================================
// PROPERTY_DISC_ITERATOR_4_3 FACET
// ===========================================================

// PUBLIC MEMBER FUNCTIONS

fields::property_disc_iterator_4_3::
property_disc_iterator_4_3(const field_vd& xfield)
{
...
get_prop_disc_values(const sec_vd& xsec)
{
  ...
  // Since the coord eval and disc are the same,
  // there should only be a single coord disc member.

  require(coordinate_discretization_members().ct() == 1);

  // Since the prop disc at or above the prop eval,
  // there should only be a single prop disc member.
...
next(bool xtruncate)
{
  ...
  while(!is_done())
  {
    if(action() == PREVISIT_ACTION)
    {
      ...
      // The prop eval is at or below the prop disc.
...
is_ancestor_of(const any* xother) const
{
  ...
  bool result = dynamic_cast<const property_disc_iterator_4_3*>(xother) != 0;
...
clone() const
{
  ...
  property_disc_iterator_4_3* result =
    new property_disc_iterator_4_3(coordinates_schema(), property_schema());
