...
// $RCSfile: field_ed.cc,v $ $Revision: 1.12 $ $Date: 2013/03/13 00:58:55 $
...
fields::field_ed::
field_ed()
  : field_at1(new sec_ed, new sec_ed_invertible, new base_space_member)
{
  // Preconditions:
...
field_ed(sec_ed* xproperty,
  ...
  : field_at1(xproperty, xcoordinates, xbase_space)
{
  // Preconditions:
...
fields::field_ed&
fields::field_ed::
operator=(const field_ed& xother)
{
  // Preconditions:
...
fields::field_ed&
fields::field_ed::
operator=(const sec_ed& xsection)
{
  // Preconditions:
...
property() const
{
  ...
  sec_ed& result = dynamic_cast<sec_ed&>(*_property);
...
bool
fields::field_ed::
same_property_fiber_schema(const field_ed& xother, bool xauto_access) const
{
  bool result;
...
static_class_name()
{
  ...
  static const string result("field_ed");
...
is_ancestor_of(const any* xother) const
{
  ...
  bool result = dynamic_cast<const field_ed*>(xother) != 0;
...
invariant() const
{
  ...
  if(invariant_check())
  {
    ...
    invariance(field_at1::invariant());
...
fields::field_ed&
fields::
operator>>(const field_ed& xsrc, field_ed& xdst)
{
  // Preconditions:
...
ostream&
fields::
operator<<(ostream& xos, const field_ed& xfield)
{
  // Preconditions:
