// $RCSfile: kd_edge_cohort.cc,v $ $Revision: 1.4 $ $Date: 2013/03/13 00:58:59 $
...
/// @file
/// Implementation for class kd_edge_cohort
...
kd_edge_cohort(kd_lattice& xkd_host, const string& xname, kd_alignment xalignment)
{
  ...
  require(xkd_host.name_space().member_poset(xkd_host.base_space().prototypes_poset_name(), false).contains_member(kd_edge::prototype_name(xalignment)));
  
  // Body:

  d_cells_cohort::new_state(xkd_host.base_space(), xname, kd_edge::prototype_name(xalignment));
  ...
  ensure(host().row_dof_tuple(dof_tuple_id(), false)->type_name == kd_edge::prototype_name(xalignment));
...
{
  // cout << endl << "Entering kd_edge_cohort::unleaf." << endl;
  ...
  _kd_host->leaves().remove_member(xid);
  ...
  // cout << "Leaving kd_edge_cohort::unleaf." << endl;
  return;
}
...
{
  // cout << endl << "Entering kd_edge_cohort::post_creation_action." << endl;
  ...
  _kd_host->edges().insert_member(xnew_mbr_id);
  ...
  ensure(kd_host().edges().contains(xnew_mbr_id));
  ensure(kd_host().leaves().contains(xnew_mbr_id));
  
  // Exit:

  // cout << "Leaving kd_edge_cohort::post_creation_action." << endl;
  return;
}
...
ostream& 
geometry::
operator << (ostream& xos, kd_edge_cohort& xn)
{
  // Preconditions:
