...
/// @file
/// Implementation for class ij_adjacency_index_space_interval
...
const fiber_bundle::ij_adjacency_index_space_interval&
	  ...
	  size_type xj_size)
{
  ...
  ij_adjacency_index_space_interval* result_ptr = new ij_adjacency_index_space_interval();
...
// ===========================================================
// IJ_ADJACENCY_INDEX_SPACE_INTERVAL FACET
// ===========================================================
...
fiber_bundle::ij_adjacency_index_space_interval::
~ij_adjacency_index_space_interval()
{  
  // Preconditions:
...
fiber_bundle::ij_adjacency_index_space_interval::pod_type
fiber_bundle::ij_adjacency_index_space_interval::
	       ...
	       size_type& xj_ct) const
{
  ...
  xi_ct = 0;
  xj_ct = 0;
  xzone_hub_begin = ordinal(i, j, _j_size) + _zone_hub_begin;

  if(i > 0)
  {
    xi_ct++;
    xzone_hub_begin -= _j_size;
  }
...
fiber_bundle::ij_adjacency_index_space_interval::
ij_adjacency_index_space_interval(const arg_list& xargs)
...
class_name() const
{
  static const string result("ij_adjacency_index_space_interval");
  return result;
}
...
clone(const arg_list& xargs) const
{
  ...
  ij_adjacency_index_space_interval* result =
    new ij_adjacency_index_space_interval(xargs);
...
make_prototype()
{
  ...
  ij_adjacency_index_space_interval* lproto = new ij_adjacency_index_space_interval();
...
operator==(const index_space_collection& xother) const
{
  ...
  const ij_adjacency_index_space_interval& lother =
    dynamic_cast<const ij_adjacency_index_space_interval&>(xother);
...
contains_unglued_hub(pod_type xlocal_id, pod_type xid) const
{
  ...
  size_type li_ct, lj_ct;

  adjacent_zones(xlocal_id, lzone_hub_begin, li_ct, lj_ct);
...
pod(pod_type xlocal_id, pod_type xid) const
{
  ...
  if((0 <= i) && (i < li_ct) && (0 <= j) && (j < lj_ct))
  {
    result = ordinal(i, j, lj_ct);
  }
  else
  {
...
unglued_hub_pod(pod_type xlocal_id, pod_type xid) const
{
  ...
  if((0 <= i) && (i < li_ct) && (0 <= j) && (j < lj_ct))
  {
    result = ordinal(i, j, _j_size) + lzone_hub_begin;
  }
  else
  {
    result = invalid_pod_index();
...
release_id_space_iterator(index_space_iterator& xitr) const
{
  ...
  _iterators.release(reinterpret_cast<ij_adjacency_implicit_index_space_iterator&>(xitr));
...
allocated_id_space_iterator(const index_space_iterator& xitr) const
{
  ...
  const ij_adjacency_implicit_index_space_iterator* litr =
    dynamic_cast<const ij_adjacency_implicit_index_space_iterator*>(&xitr);
...
is_ancestor_of(const any *other) const
{
  ...
  bool result = dynamic_cast<const ij_adjacency_index_space_interval*>(other) != 0;
...
size_t
fiber_bundle::
deep_size(const ij_adjacency_index_space_interval& xn, bool xinclude_shallow)
{
  // Preconditions:
