// $RCSfile: i_connectivity_implicit_index_space_iterator.cc,v $ $Revision: 1.4 $ $Date: 2013/01/25 01:32:12 $
...
/// @file
/// Implementation for class i_connectivity_implicit_index_space_iterator
...
// ===========================================================
// I_CONNECTIVITY_IMPLICIT_INDEX_SPACE_ITERATOR FACET
// ===========================================================

// PUBLIC MEMBER FUNCTIONS

fiber_bundle::i_connectivity_implicit_index_space_iterator::
...
fiber_bundle::i_connectivity_implicit_index_space_iterator::
i_connectivity_implicit_index_space_iterator(const i_connectivity_implicit_index_space_iterator& xother)
{
  // Preconditions:
...
fiber_bundle::i_connectivity_implicit_index_space_iterator::
i_connectivity_implicit_index_space_iterator(const index_space_family& xid_spaces,
					     pod_type xindex)
{
  // Preconditions:
...
fiber_bundle::i_connectivity_implicit_index_space_iterator&
fiber_bundle::i_connectivity_implicit_index_space_iterator::
operator=(const i_connectivity_implicit_index_space_iterator& xother)
{
  // Preconditions:
...
fiber_bundle::i_connectivity_implicit_index_space_iterator::pod_type
fiber_bundle::i_connectivity_implicit_index_space_iterator::
hub_begin() const
{
  // Preconditions:
...
operator=(const index_space_iterator& xother)
{
  ...
  const i_connectivity_implicit_index_space_iterator& lother =
    dynamic_cast<const i_connectivity_implicit_index_space_iterator&>(xother);
...
clone() const
{
  i_connectivity_implicit_index_space_iterator* result;

  // Preconditions:

  // Body:

  result = new i_connectivity_implicit_index_space_iterator(*this);
...
next()
{
  ...
  switch(_pod)
  {
    case 0:
    {
      // Increment to vertex (i+1) = _hub_pod + 1
...
attach_to(const index_space_collection& xhost, pod_type xlocal_id)
{
  ...
  // Find the section of the i_connectivity that represents this id space.

  const i_connectivity_index_space_interval* lhost =
    reinterpret_cast<const i_connectivity_index_space_interval*>(_host);
		  ...
		  pod_type xlocal_id) const
{
  ...
  bool result = (dynamic_cast<const i_connectivity_index_space_interval*>(&xhost) != 0);
...
is_ancestor_of(const any *other) const
{
  ...
  bool result = dynamic_cast<const i_connectivity_implicit_index_space_iterator*>(other) != 0;
