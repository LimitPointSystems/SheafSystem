
// $RCSfile: met_e2.impl.h,v $ $Revision: 1.10 $ $Date: 2013/01/12 17:17:02 $

//
// Copyright (c) 2013 Limit Point Systems, Inc.
//

/// @file
/// Implementation for class met_e2.

#ifndef MET_E2_IMPL_H
#define MET_E2_IMPL_H

#ifndef SHEAF_DLL_SPEC_H
#include "sheaf_dll_spec.h"
#endif

#ifndef MET_E2_H
#include "met_e2.h"
#endif

namespace fiber_bundle
{

//==============================================================================
// CLASS MET_E2_ROW_DOFS_TYPE
//==============================================================================

template <typename T>
const string&
met_e2_row_dofs_type<T>::
static_class_name()
{
  // Preconditions:

  // Body:

  static const string result("met_e2_row_dofs_type");

  // Postconditions:

  ensure(!result.empty());

  // Exit:

  return result;
}

template <typename T>
int
met_e2_row_dofs_type<T>::
d()
{
  // Preconditions:

  // Body:

  int result = 3;

  // Postconditions:

  ensure(result == 3);

  // Exit:

  return result;
}

template <typename T>
T&
met_e2_row_dofs_type<T>::
operator[] (int xindex)
{
  // Preconditions:

  require(xindex>=0 && xindex<d());

  // Body:

  T& result = components[xindex];

  // Postconditions:

  // Exit:

  return result;
}

template <typename T>
const T&
met_e2_row_dofs_type<T>::
operator[] (int xindex) const
{
  // Preconditions:

  require(xindex>=0 && xindex<d());

  // Body:

  const T& result = components[xindex];

  // Postconditions:

  // Exit:

  return result;
}

template <typename T>
met_e2_row_dofs_type<T>::
operator typename met_e2_row_dofs_type<T>::matrix_type& ()
{
  // Preconditions:

  // Body:

  matrix_type& result = reinterpret_cast<matrix_type&>(*this);

  // Postconditions:

  // Exit:

  return result;
}

template <typename T>
met_e2_row_dofs_type<T>::
operator const typename met_e2_row_dofs_type<T>::matrix_type& () const
{
  // Preconditions:

  // Body:

  met_e2_row_dofs_type<T>& lthis = const_cast<met_e2_row_dofs_type<T>&>(*this);
  const matrix_type& result = reinterpret_cast<matrix_type&>(lthis);

  // Postconditions:

  // Exit:

  return result;
}

template <typename T>
met_e2_row_dofs_type<T>* 
met_e2_row_dofs_type<T>::clone() const
{
  // Preconditions:

  // Body:

  met_e2_row_dofs_type<T>* result = new met_e2_row_dofs_type<T>();

  // Postconditions:

  ensure(result != 0);

  // Exit:

  return result;
}

//==============================================================================
// NON-MEMBER FUNCTIONS
//==============================================================================

#ifndef DOXYGEN_SKIP_IMPLEMENTATIONS
template <typename T>
ostream& operator<<(ostream& xos, const met_e2_row_dofs_type<T>& xrdt)
{
  int ld = xrdt.d();
  for(int i=0; i<ld; ++i)
  {
    xos << " " << xrdt.components[i];
  }

  return xos;
}
#endif // ifndef DOXYGEN_SKIP_IMPLEMENTATIONS

} // namespace fiber_bundle

#endif // ifndef MET_E2_IMPL_H
