...
// ===========================================================
// HOST FACTORY FACET OF CLASS SEC_E2
// ===========================================================
...
new_host(namespace_type& xns, 
         ...
         bool xauto_access)
{
  // cout << endl << "Entering sec_e2::new_host." << endl;
  ...
  // cout << "Leaving sec_e2::new_host." << endl;
  return result;
}
              ...
              bool xauto_access)
{
  ...
  require(standard_host_is_available<sec_e2>(xns, xbase_path, xrep_path, xsection_suffix, xfiber_suffix, xauto_access));
  require(fiber_type::standard_host_is_available<fiber_type>(xns, xfiber_suffix, xauto_access));
  require(schema_type::standard_host_is_available<sec_e2>(xns, xbase_path, xrep_path, xfiber_suffix, xauto_access));

  // Body:

  poset_path lstd_path = standard_host_path<sec_e2>(xbase_path, xrep_path, xsection_suffix, xfiber_suffix);
  ...
  else
  {
    ...
    poset_path lstd_schema_path = 
      schema_type::standard_member<sec_e2>(xns, xbase_path, lrep_path, xfiber_suffix, xauto_access);
  ...
  ensure(result.path(true) == standard_host_path<sec_e2>(xbase_path, xrep_path, xsection_suffix, xfiber_suffix));
  ensure(result.state_is_not_read_accessible());
  ensure(result.schema(true).path(xauto_access) == 
         schema_type::standard_member_path<sec_e2>(xbase_path, xrep_path, xfiber_suffix));
...
fiber_bundle::sec_e2::
sec_e2(const sec_rep_space* xhost, const string& xname)
{
  // Preconditions:
...
fiber_bundle::sec_e2::
sec_e2(const sec_rep_space_member* xother)
{
  // Preconditions:
...
fiber_bundle::sec_e2::
sec_e2(sec_rep_space* xhost, section_dof_map* xdof_map, bool xauto_access)
{
  // Preconditions:
...
const fiber_bundle::sec_e2::fiber_type&
fiber_bundle::sec_e2::
...
//==============================================================================
// ABSTRACT POSET MEMBER FACET OF CLASS SEC_E2
//==============================================================================
...
static_class_name()
{
  ...
  static const string result("sec_e2");
...
is_ancestor_of(const any* xother) const
{
  ...
  bool result = dynamic_cast<const sec_e2*>(xother) != 0;
...
invariant() const
{
  ...
  if (invariant_check())
  {
    ...
    invariance(is_attached() ? df() == 2 : true);
