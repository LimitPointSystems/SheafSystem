...
bool
fiber_bundle::binary_section_space_schema_member::
standard_host_is_available(namespace_type& xns, 
              ...
              bool xauto_access)
{
  ...
  require(xfiber_suffix.empty() || poset_path::is_valid_name(xfiber_suffix));
  require(typename S::fiber_type::standard_host_is_available<typename S::fiber_type>(xfiber_suffix));
  ...
  else
  {
    ...
    lresult_ptr = &new_host(xns, lstd_path, lstd_schema_path, xbase_path, xfiber_path, xrep_path, xauto_access);
  }
  ...
  ensure(result.fiber_space_path(true) == S::fiber_type::standard_host_name<S::fiber_type>(xfiber_suffix));
                ...
                bool xauto_access)
{
  ...
  require(precondition_of(standard_host(xns, xbase_path, xrep_path, xfiber_suffix, xauto_access)));
  ...
  poset_path lstd_path(standard_member_path<S>(xbase_path, xfiber_path, xrep_path));
  ...
  {  
    ...
    lmbr.put_name(lstd_path.member_name(), true, true);
  }

  // Postconditions:

  ensure(postcondition_of(standard_host(xns, xbase_path, xrep_path, xfiber_suffix, xauto_access)));

  esnure(result == standard_member_path<S>(xbase_path, xrep_path, xfiber_suffix));  
