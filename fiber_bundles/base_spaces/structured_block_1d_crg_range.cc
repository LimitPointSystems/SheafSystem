// $RCSfile: structured_block_1d_crg_range.cc,v $ $Revision: 1.24 $ $Date: 2013/01/12 17:16:52 $
...
/// @file
/// Implementation for class structured_block_1d_crg_range
...
// ===========================================================
// STRUCTURED_BLOCK_1D_CRG_RANGE FACET
// ===========================================================

// PUBLIC DATA MEMBERS

fiber_bundle::structured_block_1d_crg_range::
...
//   lcover->push_back(_vertex_id_space.offset());
//   lcover->push_back(_vertex_id_space.offset()+_vertex_id_space.ct()-1);
...
initialize_block_vertices() const
{
  ...
  _block_vertices_id_space.push_back(_vertex_id_space.offset());
  _block_vertices_id_space.push_back(_vertex_id_space.offset()+_vertex_id_space.ct()-1);
...
sheaf::structured_block_1d_crg_range::
...
finalize(poset_state_handle& xhost)
{
  ...
  size_type lzone_ct = _i_size;
  size_type lvertex_ct = _i_vertex_size;
  ...
  const string& lzone_id_space_name =
    section_space_schema_member::intersection_id_space_name(lhost.d_cells(1), lblock_id);
  ...
  // Construct block vertices id space.
  ...
  arg_list largs =
    i_connectivity_index_space_interval::make_arg_list(lvertex_begin);

  initialize_connectivity_id_spaces("i_connectivity_index_space_interval",
				    largs, lzone_ct);

  // Construct the adjacency id space interval.

  largs = i_adjacency_index_space_interval::make_arg_list(lzone_begin, _i_size);
  initialize_adjacency_id_spaces("i_adjacency_index_space_interval",
				 largs, lvertex_ct);
...
class_name() const
{
  static const string result("structured_block_1d_crg_range");
  return result;
}
...
make_prototype()
{
  ...
  structured_block_1d_crg_range* lproto = new structured_block_1d_crg_range();
...
is_ancestor_of(const any* other) const
{
  ...
  bool result = dynamic_cast<const structured_block_1d_crg_range*>(other) != 0;
...
clone() const
{
  ...
  structured_block_1d_crg_range* result = new structured_block_1d_crg_range();
...
invariant() const
{
  ...
  if(invariant_check())
  {
    ...
    invariance(zones_initialized() ? zone_id_space().ct() == i_size() : true);
    invariance(vertices_initialized() ? vertex_id_space().ct() == i_size()+1 : true);
...
size_t
fiber_bundle::
deep_size(const structured_block_1d_crg_range& xr, bool xinclude_shallow)
{
  size_t result;
