...
#include "ComLimitPoint/fiber_bundle/gl2.impl.h"
         ...
         bool xauto_access)
{
  ...
  require(schema_poset_member::row_dof_ct(xns, xschema_path, xauto_access) == 8);
  ...
  ensure(result.n(true) == 2);
  ensure(result.vector_space_path(true) == xvector_space_path );

  ensure(result.d(true) == 8);
  
  // Exit:

  // cout << "Leaving gl2::new_host." << endl;
  return result;
}
...
standard_host(namespace_type& xns, const std::string& xsuffix, bool xauto_access)
{
  ...
  require(standard_host_is_available<gl2>(xns, xsuffix, xauto_access));
  ...
  poset_path lpath(standard_host_path<gl2>(xsuffix));
  ...
  ensure(result.path(true) == standard_host_path<gl2>(xsuffix));
  ensure(result.state_is_not_read_accessible());
