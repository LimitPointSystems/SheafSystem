...
#include "ComLimitPoint/fiber_bundle/structured_block_1d_crg_interval.h"
...
~structured_block_1d_crg_interval()
{
  ...
  if(is_valid(_lower_covers_begin))
  {
    ...
    _id_spaces->delete_space(_lower_covers_begin + 1 + _i_size);
  }
  ...
  if(is_valid(_upper_covers_begin))
  {
    ...
    _id_spaces->delete_space(_upper_covers_begin + 1 + _i_size);
  }
                  ...
                  block<pod_index_type>& result)
{
  ...
  result[BLOCK]  = structured_block_1d::new_row_dof_map(xhost, xi_size, true).hub_pod();
  result[ZONE]   = base_space_member::new_row_dof_map(xhost, "segment_complex", true).hub_pod();
