...
/// @file
/// Implementation for class point_block_3d
...
#include "ComLimitPoint/fiber_bundle/point_block_crg_interval.h"
...
static_prototype_path()
{
  ...
  STATIC_PROTOTYPE_PATH(base_space_member::prototypes_poset_name(), "point_block_3d");
  ...
  ensure(result.member_name() == "point_block_3d");
...
new_host(namespace_type& xns, const poset_path& xhost_path, const poset_path& xschema_path, bool xauto_access)
{
  ...
  // cout << "Leaving point_block_3d::new_host." << endl;
  return result;
}
...
{
  // cout << endl << "Entering point_block_3d::standard_host." << endl;
  ...
  // cout << "Leaving point_block_3d::standard_host." << endl;
  return result;
}
...
fiber_bundle::point_block_3d::
point_block_3d(const abstract_poset_member& xother, bool xnew_jem)
{
  // Preconditions:
          ...
          bool xauto_access)
{
  ...
  size_type lsize;
  point_block_crg_interval::get_size(xi_size, xj_size, xk_size, lsize);

  // Get the dof tuple_ids.

  block<pod_index_type> ldof_tuple_ids;
  point_block_crg_interval::get_dof_tuple_ids(*xhost, xi_size, xj_size, xk_size, ldof_tuple_ids);

  // Get the private data for the interval

  block<pod_index_type> ldata;
  point_block_crg_interval::get_private_data(xi_size, xj_size, xk_size, ldata);
...
i_size() const
{
  ...
  point_block_3d* cthis = const_cast<point_block_3d*>(this);
...
k_size() const
{
  ...
  point_block_3d* cthis = const_cast<point_block_3d*>(this);
  const size_type& result = cthis->row_dof_tuple()->k_size;
...
void
fiber_bundle::point_block_3d::
put_k_size(const size_type& xsize)
{
  // Preconditions:
                ...
                bool xauto_access)
{
  ...
  size_type lsize = xi_size*xj_size*xk_size;
  ...
  // Set the size in the i, j and k directions.
...
invariant() const
{
  ...
  if(invariant_check())
  {
    ...
    invariance(state_is_read_accessible() ?
               size() == (i_size()*j_size()*k_size()) : true);
...
is_ancestor_of(const any* xother) const
{
  ...
  result = dynamic_cast<const point_block_3d*>(xother) != 0;
...
fiber_bundle::point_block_3d&
fiber_bundle::point_block_3d::
operator=(const point_block_3d& xother)
{
