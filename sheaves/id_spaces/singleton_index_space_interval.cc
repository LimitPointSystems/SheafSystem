...
const sheaf::singleton_index_space_interval&
	  ...
	  pod_type xhub_offset)
{
  ...
  singleton_index_space_interval* result_ptr = new singleton_index_space_interval();
  result_ptr->new_state(xid_spaces, xub);

  result_ptr->_hub_offset = xhub_offset;

  const singleton_index_space_interval& result = *result_ptr;
  ...
  ensure(result.hub_offset() == xhub_offset);
