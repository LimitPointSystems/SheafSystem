// $RCSfile: singleton_implicit_index_space_iterator.cc,v $ $Revision: 1.4 $ $Date: 2013/01/25 01:32:12 $
...
sheaf::singleton_implicit_index_space_iterator&
sheaf::singleton_implicit_index_space_iterator::
operator=(const singleton_implicit_index_space_iterator& xother)
{
  // Preconditions:
...
operator=(const index_space_iterator& xother)
{
  ...
  const singleton_implicit_index_space_iterator& lother =
    dynamic_cast<const singleton_implicit_index_space_iterator&>(xother);
...
clone() const
{
  singleton_implicit_index_space_iterator* result;

  // Preconditions:

  // Body:

  result = new singleton_implicit_index_space_iterator(*this);
...
attach_to(const index_space_collection& xhost, pod_type xlocal_id)
{
  ...
  // Find the section of the singleton that represents this id space.

  const singleton_index_space_interval* lhost =
    reinterpret_cast<const singleton_index_space_interval*>(_host);

  _hub_begin = xlocal_id + lhost->hub_offset();
		  ...
		  pod_type xlocal_id) const
{
  ...
  bool result = (dynamic_cast<const singleton_index_space_interval*>(&xhost) != 0);
...
is_ancestor_of(const any *other) const
{
  ...
  bool result = dynamic_cast<const singleton_implicit_index_space_iterator*>(other) != 0;
