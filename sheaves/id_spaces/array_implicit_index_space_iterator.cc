// $RCSfile: array_implicit_index_space_iterator.cc,v $ $Revision: 1.4 $ $Date: 2013/01/25 01:32:12 $
...
/// @file
/// Implementation for class array_implicit_index_space_iterator
...
sheaf::array_implicit_index_space_iterator::
...
sheaf::array_implicit_index_space_iterator&
sheaf::array_implicit_index_space_iterator::
operator=(const array_implicit_index_space_iterator& xother)
{
  // Preconditions:
...
operator=(const index_space_iterator& xother)
{
  ...
  const array_implicit_index_space_iterator& lother =
    dynamic_cast<const array_implicit_index_space_iterator&>(xother);
...
clone() const
{
  array_implicit_index_space_iterator* result;

  // Preconditions:

  // Body:

  result = new array_implicit_index_space_iterator(*this);
...
attach_to(const index_space_collection& xhost, pod_type xlocal_id)
{
  ...
  require(reinterpret_cast<const array_index_space_interval&>(xhost).ids_per_space() > 0);
  ...
  // Find the section of the array that represents this id space.

  const array_index_space_interval* lhost =
    reinterpret_cast<const array_index_space_interval*>(_host);

  _to_hub   = &lhost->hub_ids()[_local_id * lhost->ids_per_space()];
  _end      = lhost->ids_per_space();
		  ...
		  pod_type xlocal_id) const
{
  ...
  bool result = (dynamic_cast<const array_index_space_interval*>(&xhost) != 0);
...
is_ancestor_of(const any *other) const
{
  ...
  bool result = dynamic_cast<const array_implicit_index_space_iterator*>(other) != 0;
